	...
	render_sidemenu: function () {
        ...
        frappe.call({
                ...
                callback:function(list){
                    var buttons = '<div class="layout-main-section hidden-sm hidden-xs">';
					if (list["message"]){
					me.account = list["message"][0]["email_account"];
					buttons += '<div class="list-row list-row-head" style="font-weight:bold"> <div class="row"><span class="inbox-select col-md-12 " style="margin-left: 10px;">'+list["message"][0]["email_account"]+'</span> </div></div>';
                    for (var i = 1;i<list["message"].length;i++)
                    {
						buttons += '<div class="list-row"> <div class="row"><span class="inbox-select col-md-12 " style="margin-left: 10px;">'+list["message"][i]["email_account"]+'</span> </div></div>';
                    }
					me.wrapper.page.sidebar.append(buttons);
					$(".inbox-select").click(function(btn){
						me.account = btn.currentTarget.innerHTML;
	...
	render_list: function(){
        ...
        frappe.call({
                ...
                args:{
					...
					start:me.cur_page*me.page_length,
	...
	render_footer:function(){
		...
		frappe.call({
                ...
                callback: function (r) {
					...
					me.last_page = Math.floor(r.message[0][0]/me.page_length);
